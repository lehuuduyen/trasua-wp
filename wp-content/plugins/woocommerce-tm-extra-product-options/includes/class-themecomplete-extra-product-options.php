...
final class THEMECOMPLETE_Extra_Product_Options {
	...
	public function add_plugin_actions() {
		...
		add_action( 'woocommerce_before_calculate_totals', [ $this, 'weight_woocommerce_checkout_update_order_review' ], 1 );
								...
								if ( $element_object ) {
									...
									if ( 'single' === $element_object->type || 'multipleallsingle' === $element_object->type ) {
										...
										if ( 'lookuptable' === $this_price_type ) {
											...
											if ( false !== $table_name ) {
												$table_num = trim( substr( $lookuptable, ( 0 - ( strlen( $table_name ) - 1 ) ) ) );
											} else {
												$table_num = '0';
											}
