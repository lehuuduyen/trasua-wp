...
function handleConvertMoney(e) {
  ...
  const spanEditAll = document.getElementById(
    "points_converted_to_money_span_edit_all"
  );
...
function showEditAll() {
  document.getElementById("button-open-modal-edit-all").style.cssText =
    "display: flex";
  document
    .getElementById("button-open-modal-edit-all")
    .classList.remove(hiddenClass);
  document.getElementById("button-open-modal-add").style.cssText =
    "display: flex";
  document
    .getElementById("button-open-modal-add")
    .classList.remove(hiddenClass);
...
function changeModalContentEdit() {
  for (let i = 1; i <= 4; i++) {
    const contentStep = document.getElementsByClassName(
      `content-step-${i}-edit`
    );
    if (i === step) {
...
function handleInputChange(modal) {
  if (modal) {
    ...
    const minimumSpending = modal.querySelectorAll(
      'input[name="minimum_spending[]"]'
    );
    const showMinimumSpendingFinal = modal.querySelectorAll(
      ".show-minimum-spending-final"
    );
    handleChangeValue(minimumSpending, showMinimumSpendingFinal);

    /* Handle price sale off */
    const priceSaleOff = modal.querySelectorAll(
      'input[name="price_sale_off[]"]'
    );
    const showPriceSaleOffFinal = modal.querySelectorAll(
      ".show-price-sale-off-final"
    );
    handleChangeValue(priceSaleOff, showPriceSaleOffFinal);

    /* Handle price sale off max */
    const priceSaleOffMax = modal.querySelectorAll(
      'input[name="price_sale_off_max[]"]'
    );
    const showPriceSaleOffMaxFinal = modal.querySelectorAll(
      ".show-price-sale-off-max-final"
    );
    ...
    const priceSaleOffMaxByClass = modal.querySelectorAll(
      ".price-sale-off-max"
    );
...
function openEditPointModal(id) {
  const modal = document.getElementById(`modal-edit-point-${id}`);


  const modals = document.querySelectorAll(".modal-edit-point");
...
function openEditModal(id) {
  ...
  const modalNextStep2 = document.getElementById(
    `modal-next-edit-step-2-${id}`
  );
  const modalNextStep3 = document.getElementById(
    `modal-next-edit-step-3-${id}`
  );
  ...
  !ids.includes(id) &&
    modalNextStep2.addEventListener("click", function () {
      ...
      if (step === 2) {
        ...
        const requireInputStep2 =
          contentStep2.querySelectorAll(".require-field");
  ...
  !ids.includes(id) &&
    modalNextStep3.addEventListener("click", function () {
      ...
      if (step === 3) {
        ...
        const requireInputStep3 =
          contentStep3.querySelectorAll(".require-field");
        ...
        if (
          checkPassStep3 === requireInputStep3.length ||
          (checkPassStep3 === 1 && limitInput.checked === false)
        ) {
          nextStep();
        }
      }
  ...
  !ids.includes(id) &&
    modalPrev.addEventListener("click", function () {
      document
        .getElementById(`step-${step}-edit-${id}`)
    ...
    uploadImageBtn.addEventListener("click", function (e) {
      ...
      uploader = wp
        ...
        .on("select", function () {
          ...
          document.getElementById(`image-url-edit-${id}`).value =
            attachment.url;
...
function openEditAllModal() {
  ...
  document
    .getElementById(`step-${stepEditAll}-edit-all`)
  ...
  triggerEditAll <= 1 &&
    modalNextStep1.addEventListener("click", function () {
      ...
      if (stepEditAll === 1) {
        ...
        const requireInputStep1 =
          contentStep1.querySelectorAll(".require-field");
    ...
    modalNextStep3.addEventListener("click", function () {
      ...
      trStep3.forEach((tr) => {
        ...
        if (limitInput.checked) {
          if (
            requireInputLimitStep3.value === "" ||
            requireInputLimitStep3.value < 0
          ) {
            requireInputLimitStep3.nextElementSibling.classList.remove(
              hiddenClass
            );
          } else {
            requireInputLimitStep3.nextElementSibling.classList.add(
              hiddenClass
            );
            checkStep3++;
          }
  ...
  const allButtonImageUpload = step2Wrapper.querySelectorAll(
    ".upload-image-button-edit-all"
  );
  ...
  const numberOfRanking =
    document.getElementsByName("number_of_ranking")[0].value;
  ...
  deleteEditRecord.forEach((item) => {
    item.addEventListener("click", function () {
      ...
      const deleteEditRecordCheck = document.querySelectorAll(
        ".delete-edit-record"
      );
...
window.addEventListener("load", function () {
    ...
    `;
      ...
      deleteAddRecord.forEach((item) => {
        item.addEventListener("click", function () {
          ...
          item.parentElement.parentElement.remove();
          const step3Delete = document.querySelector(
            `#record-step-3-add-${currentId}`
          );
          const step4Delete = document.querySelector(
            `#record-step-4-add-${currentId}`
          );
          step3Delete && step3Delete.remove();
          step4Delete && step4Delete.remove();

          const deleteAddRecordCheck =
            document.querySelectorAll(".delete-add-record");
  ...
  function reTriggerUploadImage(parent, number) {
    ...
    uploadImageBtn.addEventListener("click", function (e) {
      ...
      uploader = wp
        ...
        .on("select", function () {
          const attachment = uploader.state().get("selection").first().toJSON();
          const tdRecord = document.getElementById(
            `record-step-2-add-${number}`
          );
  ...
  checkboxAll &&
    checkboxAll.addEventListener("click", function (e) {
      ...
      listIdsDelete = [...new Set(listIdsDelete.map(JSON.stringify))].map(
        JSON.parse
      );
    });
  ...
  listCheckbox.forEach((item) => {
    item.addEventListener("click", function () {
      if (item.checked) {
        ...
        listIdsDelete = listIdsDelete.filter(
          ({ id }) => Number(id) !== Number(item.value.split("-")[0])
        );
        check = 0;
    ...
    modalNextStep3.addEventListener("click", function () {
      ...
      if (step === 3) {
        ...
        trStep3.forEach((tr) => {
          let checkStep3 = 0;
          const requireInputStep3 = tr.querySelector(".require-field");
          const requireInputLimitStep3 = tr.querySelector(
            ".require-field-limit"
          );
          const limitInput = tr.querySelector(".is-limit-input");
  ...
  function changeModalContent() {
    for (let i = 1; i <= 4; i++) {
      if (i === step) {
        document
          .getElementsByClassName(`content-step-${i}`)[0]
          .classList.remove(hiddenClass);
      } else {
        document
          .getElementsByClassName(`content-step-${i}`)[0]
