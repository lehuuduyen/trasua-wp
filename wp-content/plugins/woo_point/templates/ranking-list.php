              ...
              <table class="wp-list-table widefat striped table-view-list">
                  ...
                  <tbody>
                    <tr>
                      ...
                      <?php 
                       $settingsEdit = array(
                        'media_buttons' => true, // Show media insert/upload buttons
                        'textarea_rows' => 10,   // Number of rows in the textarea
                        'teeny'         => false, // Output the minimal editor config
                        'tinymce'       => array(
                            'plugins' => 'wordpress',
                        ),
                    );
                      wp_editor( $value['text'], 'text'.$value['id'], $settingsEdit); ?>
                    </form>
                      <!-- <textarea class="require-field message_editor"   name="text[]" placeholder="Vui lòng nhập Message" ><?php echo $value['text']; ?> </textarea>
                        <p class="form-error-text d-none">Đây là trường bắt buộc</p> -->
                      </td>
                      <td>
        ...
        <div class="step-content content-step-1-edit-all">
          ...
          <div class="step-content-content" style="display: flex; gap: 20px;">
            <div>
              ...
              <p>Khách hàng sẻ đổi 1 điểm để được <span id="points_converted_to_money_span"><?php echo count($settings) > 0 ? $settings[0]['points_converted_to_money'] : ''; ?></span>đ Chi tiêu</p>
            </div>
            <div>
              <h4>Tỉ lệ tích điểm theo chỉ tiêu</h4>
              <div class="group-input" style="display: flex; gap: 15px;">
              ...
              <p>Khách hàng sẻ nhận được 1 điểm khi thanh toán <span id="amount_spent_span"><?php echo count($settings) > 0 ? $settings[0]['amount_spent'] : ''; ?></span>đ </p>
            </div>
          </div>
        </div>
                ...
                <?php foreach ($ranks as $key => $value) { ?>
                  <tr id="record-step-3-edit-all-<?php echo $value['id']; ?>">
                    ...
                    <form>
                      ...
                      //   ),
                    // );
                      // wp_editor( $value['text'], 'text'.$key, $settingsEdit); ?>
                    </form>
                      <textarea class="require-field message_editor"  name="text[]" placeholder="Vui lòng nhập Message" ><?php echo $value['text']; ?> </textarea>
                      <p class="form-error-text d-none">Đây là trường bắt buộc</p>
                    </td> -->
