<?php
  ...
  if (isset($_POST['addRanking'])) {
    ...
    for ($i = 0; $i < count($_POST['name']); $i++) {
      ...
      $arrayInsert = array(
        ...
        'discount' => (isset($_POST['discount'][$i]))?$_POST['discount'][$i]:0,
        'text' => $_POST['text'],
  ...
  if (isset($_POST['editRanking'])) {
    ...
    for ($i = 0; $i < count($_POST['name']); $i++) {
      $_POST['text'.$_POST['rankId']] = stripslashes($_POST['text'.$_POST['rankId']]);
  ...
  if (isset($_POST['editAllRanking'])) {
    for ($i = 0; $i < count($_POST['name']); $i++) {
      // $_POST['text'][$i] = stripslashes($_POST['text'][$i]);
...
<script>
   function loadCk(){
    const editorElements = document.querySelectorAll('.message_editor');

      editorElements.forEach((element, index) => {
          ClassicEditor.create(element, {
            ckfinder: {
            uploadUrl: '/upload?command=QuickUpload&type=Files&responseType=json'
          }
          })
          .then(editor => {
              console.log(`Editor ${index + 1} was initialized`, editor);
          })
          .catch(error => {
              console.error(`Oops, something went wrong with editor ${index + 1}!`, error);
          });
      });
   }
